...
class SparseMatrix:
    ...
    def transposeRightMult(self, denseMat):
        ...
        while ptr != self.pointer:
            ...
            while ptr != self.pointer and rowIndex == self.rows[0, ptr]:
                for t in range(ell):
                    product[t, self.cols[0, ptr]] += (
                        self.values[0, ptr] * denseMat[rowIndex, t]
                    )
                ptr += 1
    ...
    def leftMult(self, denseMat):
        ...
        for ptr in range(self.pointer):
            # case 1
            if (
                self.rows[0, ptr] != rowIndex and ptr != self.pointer - 1
            ):  # headPtr -> ptr-1 is one row
                for j in range(headPtr, ptr):
                    product[rowIndex, :] += (
                        denseMat[self.cols[0, j], :] * self.values[0, j]
                    )
                # resetting
                rowIndex = self.rows[0, ptr]
                headPtr = ptr
                ...
                if self.rows[0, ptr] == rowIndex:
                    ...
                    rowIndex = self.rows[0, ptr]
                    product[rowIndex, :] = (
                        denseMat[self.cols[0, ptr], :] * self.values[0, ptr]
                    )
